...
#' 
#' Now the followingg variables are initialized:
...
wrapper.run <- function(moduleToExecute = NULL, jdbcDriver = NULL, jdbcUrl = NULL, 
	...
	# load the module 
	source(file.path(wrapper.scriptPath(), moduleToExecute))
  ...
  errors <- errors[!is.na(errors)]
	...
	assign('wrapper.tableNames', c(), envir = .GlobalEnv)
  ...
  # output list of produced tables
  cat(paste(wrapper.tableNames, collapse = ","))
}
...
wrapper.saveDataFrame <- function(dataFrame, tableName, rowNumbers = FALSE, incremental = FALSE, forcedColumnTypes) {
    ...
    assign('wrapper.tableNames', unique(c(wrapper.tableNames, tableName)), envir = .GlobalEnv)
}
